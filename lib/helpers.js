...
async function clickSearchPTP(page) {
  ...
  await page.waitForURL(/\/search\/point-to-point/, { timeout: 35000 })
    .catch(async () => {
      // Staying on /home means the form refused to submit, so report WHY: the most likely
      // causes are a silently-reset traveller count or an inline validation message.
